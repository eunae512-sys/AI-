...
**Domain:** Internal AI Marketing Operations Platform (Korean SMB ad agency)
**Researched:** 2026-05-10
**Confidence:** HIGH (core stack), MEDIUM (Korean-specific LLM choice)
...
This is the highest-stakes decision. Korean copy quality is core value per PROJECT.md.
...
1. **Anthropic prompt caching** with explicit `cache_control` on system prompt + brand kit + regulatory guardrails (cache is ~10x cheaper).
2. **OpenAI automatic caching** — order prompts: stable prefix first, dynamic user input last.
3. **Per-request cost logging** to Langfuse with `client_id`, `model`, `cached_tokens`, `output_tokens`. Surface "this client costs you ₩X/month" in admin UI.
4. **Embedding-based reuse**: before generating, search pgvector for similar past briefs at >0.85 cosine — reuse + diff is cheaper than generating from scratch and matches PROJECT.md's "같은 패턴이 반복됨" insight.
5. **Tiered routing**: route by task class (above table), not "always use the best model".

---

## Image Generation Strategy

Korean ad creative has two distinct needs and **no single model wins both**.
...
**Critical Korean validation (do in Phase 1):** generate 50 sample creatives with each model containing Hangul text (event names, prices, CTAs). Score readability on a 1–5 scale. Pick winner empirically — text-rendering benchmarks are heavily English-biased.

**Always render Hangul text as overlay (HTML/Canvas) when possible** rather than baking into the image — gives editable, perfectly-rendered output. Use AI-generated image as background only.

---

## Compliance & Guardrails (Korean regulatory context)

Per PROJECT.md "의료·금융 등 광고 규제 업종" and December 2025 KFTC AI-ad labeling rules:
...
**Important:** These are not optional — KFTC enforcement (96,700 illegal ads in 2024) targets exactly the food/health/beauty SMB verticals this product serves.
...
**Korean data residency note:** PIPA does NOT require Korea-only residency in general. K-ISMS-P certification path matters only if you sell into government/enterprise. Tokyo region is acceptable for v1.
...
```bash
# Project init
pnpm create next-app@latest adops-ai --typescript --tailwind --app --src-dir --import-alias "@/*"
cd adops-ai
...
**If Korean cultural nuance / Naver SEO copy quality is the #1 success metric:**
...
**If budget pressure forces single-provider at start:**
...
**If client requires 100% data-in-Korea:**
...
**If image generation cost spirals:**
...
**If user count grows past initial 5–20 (multiple agencies):**
...
1. **Korean IME in Next.js forms** — `onChange` fires mid-composition. Use `onCompositionStart/End` to debounce, especially for AI-trigger-on-type features. Tiptap handles this; bare `<textarea>` does not.
2. **Anthropic cache_control placement** — only the LAST 4 blocks marked `cache_control` are cached. Order: `[system → brand kit → regulatory rules → cache_control] [user input]`.
3. **Supabase RLS + Drizzle** — Drizzle queries must run with the user's JWT context. Use `supabase-js` for auth-scoped reads, Drizzle for service-role admin operations. Document the boundary.
4. **Vercel icn1 region** — only available on Pro plan. Lock function regions in `vercel.json` or pay for unintended cross-region traffic.
5. **Korean date/time** — agencies operate 09:00 KST; never default to UTC for any campaign-scheduled feature. `Asia/Seoul` everywhere.
6. **Pretendard subset** — full font is ~3MB. Use `Pretendard-Variable.subset.woff2` (Korean + Latin only) → ~400KB.
7. **PDF generation with Korean** — `@react-pdf/renderer` requires explicit font registration; bundle Pretendard. Default fonts won't render Hangul.
...
---
*Stack research for: Korean SMB ad agency internal AI marketing ops platform*
*Researched: 2026-05-10*
